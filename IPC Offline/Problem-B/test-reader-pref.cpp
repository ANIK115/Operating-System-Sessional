...
void* Reader(void* arg)
{
	...
	// printf("Reader: %d is trying to acquire the lock\n", threadNUmber);
	// Occupying the Lock
	ReaderLock(&rwlock);

	pthread_spin_lock(&spinlock);
	readerAcquireTime[threadNUmber] = timeIndex;
	timeIndex++;
	...
	pthread_spin_lock(&spinlock);
	readerReleaseTime[threadNUmber] = timeIndex;
	timeIndex++;
	pthread_spin_unlock(&spinlock);

	// printf("Reader: %d is trying to release the lock\n",threadNUmber);
	// Releasing the Lock
	ReaderUnlock(&rwlock);
...
void* Writer(void* arg)
{
	...
	// printf("Writer: %d is trying to acquire the lock\n", threadNUmber);
	// Occupying the Lock
	WriterLock(&rwlock);

	pthread_spin_lock(&spinlock);
	writerAcquireTime[threadNUmber] = timeIndex;
	timeIndex++;
	pthread_spin_unlock(&spinlock);
	...
	pthread_spin_lock(&spinlock);
	writerReleaseTime[threadNUmber] = timeIndex;
	timeIndex++;
	pthread_spin_unlock(&spinlock);

	// printf("Writer: %d is trying to release the lock\n",threadNUmber);
	// Releasing the Lock
	WriterUnlock(&rwlock);
